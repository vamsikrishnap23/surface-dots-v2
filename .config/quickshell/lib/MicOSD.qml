...
PanelWindow {
    ...
    WlrLayershell.namespace:     "mic-osd"
    ...
    FileView {
        id: micWatcher
        path:         Quickshell.env("HOME") + "/.cache/quickshell/mic"
        watchChanges: true
        preload:      true
    ...
    Timer {
        ...
        onTriggered: {
            ...
            setVolProc.command = ["pamixer", "--default-source", "--set-volume", pendingVol.toString()]
    ...
    Item {
        ...
        Connections {
            target: root
            function onVolumeChanged() { micIconPulse.restart() }
            function onMutedChanged()  { micIconPulse.restart() }
        }
    }
}
